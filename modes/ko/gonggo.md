...
- **경력 밴드 (`_shared.md` § 경력 밴드 게이트):** `profile.yml` → `experience.years`(또는 `months`)를 읽는다. JD **필수** 최소 연차 `M`이 `years + skip_tolerance_years`(기본 0.5)를 넘거나, 시니어 단독 리드/아키텍트/팀장 소유권이 필수인데 `years < senior_min_years`(기본 5)이면 N년차 행은 `critical (stated)` + `❌ Missing`. 개인 출시 프로젝트는 Evidence에 스킬 근거로만 쓰고 연차를 부풀리지 않는다. 우대 연차는 `preferred` gap이지 SKIP이 아니다. 연차 미기재면 이 게이트를 적용하지 않는다.
...
1. JD 레벨 vs `experience.years` (프로젝트 기간 ≠ 경력 연차)
2. 거짓 없이 senior 포지셔닝 — `years < senior_min_years`이면 시니어/리드로 포장하지 않음
