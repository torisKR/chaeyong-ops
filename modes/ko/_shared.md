...
| 공고 표현 | 평가 방식 |
|-----------|-----------|
| **N년차 이상** | `cv.md` 재직 기간만 연차로 센다. 인턴·아르바이트는 보통 제외. 개인 출시 프로젝트는 연차에 더하지 않음 |
...
채용옵스 기본 타깃은 주니어 1–2년(예: 1년 7개월). **경력 연차와 프로젝트 시니어티를 둘 다 게이트로 본다.**

**연차 산정:** `cv.md` / `config/profile.yml`에 적힌 재직 기간만. 인턴·아르바이트는 제외. 연차를 확정할 수 없으면 이 게이트를 적용하지 않고 "연차 미기재"로 표시한다 — 추정 금지.

**프로젝트는 연차가 아니다.** 개인·사이드·출시 프로젝트는 Block B 스킬 매치를 보강할 수 있다. 그 기간을 경력 연차에 더하거나, 그것으로 3년+ 필수 요건을 충족했다고 쓰지 않는다.

문서상 경력이 **2년 미만**일 때:

| JD 신호 (must-have / 필수) | 처리 |
|----------------------------|------|
| 경력 3년+ (`3년 이상`, `경력 3년`, `3~5년`, `4~7년`, `5년 이상` 등) | 경력 핏 **≤ 2/5**, 지원 **SKIP** 권고. 글로벌 점수 4.0 미만 |
| 시니어 단독 소유권 — 프로젝트 리드 / 아키텍트 / 팀장 책임을 필수로 요구 | 동일 (저점수 + SKIP) |
| 제목만 `시니어` / `Senior` / `리드` / `Lead` / `팀장` | 스캔 `title_filter.negative`가 1차 게이트. 평가에 들어왔으면 위와 동일 |

**우대 / preferred** 3년+ 또는 시니어 경험은 hard SKIP이 아니다. gap으로 표시하고 나머지 핏으로 점수를 낸다.

중·시니어를 타깃하면 `modes/_profile.md`에서 이 게이트를 끄거나 밴드를 바꾸고, `portals.yml`의 연차·시니어 제목 negative를 제거한다.
