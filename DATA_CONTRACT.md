...
| `portals.yml` | Your customized company list (including optional `blocked_companies` / `exclude_companies` skip-list) |
| `config/plugins.yml` | Your plugin activation toggles (opt-in; seeded from `config/plugins.example.yml`) |
| `config/integrations.yml` | Optional hiring-ops alert toggles/thresholds (opt-in; copy `config/integrations.example.yml`). Secrets stay in `.env`, never here |
...
| `data/blacklist.md` | Your do-not-apply company list (opt-in — absence = no filtering; never auto-populated: only you, or the agent on your explicit instruction, write to it. Respected by `scan.mjs` and the `auto-pipeline`/`oferta`/`apply` gates; never a scoring input). Same skip-list can also live as `blocked_companies` in `portals.yml` |
...
| `config/integrations.example.yml` | Optional Slack/Discord/Telegram alert toggles (seed for `config/integrations.yml`; secrets stay in `.env`) |
| `integrations/` | Webhook notifiers (Slack/Discord/Telegram) + Notion/Jira stubs |
...
| `dashboard/*` | Go dashboard: TUI plus loopback HTTP status board (`--web`) |
