...
resolve_prompt_file() {
  local resolved="$BATCH_DIR/batch-prompt.md"
  if [[ -f "$PROFILE_FILE" && -f "$BATCH_DIR/batch-prompt.ko.md" ]]; then
    local lang
    lang="$(node -e "
      const fs=require('fs'); const yaml=require('js-yaml');
      try {
        const p=yaml.load(fs.readFileSync(process.argv[1],'utf8'));
        process.stdout.write(String(p?.language?.output || 'en').trim());
      } catch { process.stdout.write('en'); }
    " "$PROFILE_FILE" 2>/dev/null || echo en)"
    if [[ "$lang" == "ko" ]]; then
      resolved="$BATCH_DIR/batch-prompt.ko.md"
    fi
  fi
  printf '%s' "$resolved"
}
